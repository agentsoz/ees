 ...
 *
 */
public class SafeRoutePlan extends Plan {
	...
	public SafeRoutePlan(Agent agent, Goal goal, String name) {
	...
	public boolean context() {
		...
		for (String r : evacController.getRoutes().keySet()) {
			safeRoute = evacController.getRoutes().get(r);
			if (safeRoute.getRegion() == region.getName()
					&& safeRoute.getReliefCentre() == reliefCentre
					&& safeRoute.getDescription() == "SAFE") {
				return true;
			}
		}
	...
	PlanStep[] steps = new PlanStep[] { new PlanStep() {
		...
		public void step() {
			evacController.addRouteAssignment(new RouteAssignment(region
					.getName(), reliefCentre, safeRoute.getName(), safeRoute
					.getRouteCoords(), safeRoute.getRouteNames()));
		}
	} };
}
