 ...
 *
 */
public class CustomRoutePlan extends Plan {
	...
	public CustomRoutePlan(Agent agent, Goal goal, String name) {
	...
	public boolean context() {
		...
		for (String r : evacController.getRoutes().keySet()) {
			customRoute = evacController.getRoutes().get(r);
			if (customRoute.getRegion() == region.getName()
					&& customRoute.getReliefCentre() == reliefCentre
					&& customRoute.getDescription() == "CUSTOM") {
				return true;
			}
		}
	...
	PlanStep[] steps = new PlanStep[] { new PlanStep() {
		...
		public void step() {
			evacController.addRouteAssignment(new RouteAssignment(region
					.getName(), reliefCentre, customRoute.getName(),
					customRoute.getRouteCoords(), customRoute.getRouteNames()));
		}
	} };
