...
gz<-'../../../../../archetypes-modelling/population/archetypes/Bendigo-SA4/Castlemaine/persons_archetypes_attributes_baseline.csv.gz'
con<-gzfile(gz,'rt')
persons1<-read.csv(con,header=T,sep=',',stringsAsFactors = F,strip.white = T)
close(con)

gz<-'../../../../../archetypes-modelling/population/archetypes/Bendigo-SA4/Castlemaine Region/persons_archetypes_attributes_baseline.csv.gz'
con<-gzfile(gz,'rt')
...
# get the same number of agents as in the original archtypes csv file
ori_persons_gz <- 'population-archetypes.csv.gz'
ori_persons <-read.csv(gzfile(ori_persons_gz,'rt'),header=T,sep=',',stringsAsFactors = F,strip.white = T)
df <- df[sample(nrow(df), nrow(ori_persons), replace = FALSE), ]
...
# 1. get the same number of dependants and randomly pick and assign agents to pickup deps
df$HasDependents <- 0 # initialise has deps to 0 (EI types have deps initially assigned)
df$HasDependentsAtLocation<-""
num_deps <- nrow(ori_persons[ori_persons$HasDependents=="true", ])
df[sample(nrow(df),num_deps,replace = FALSE), "HasDependents"] <-  1
cat(paste0(length(which(df$HasDependents==1))), " are assigned to pickup dependants")
...
# Write out the table
con <- gzfile('./population-archetypes_baseline.csv.gz')
